...
*/
    ...
    void DEBUG_INLINE convertImageYuyvToRgb(const TrikCvImageBuffer& _inImage, TrikCvImageBuffer& _outImage)
    {
      ...
      const unsigned char* restrict y_in_sobel;
      unsigned char* restrict   sobel_out;


if(m_inImageDesc.m_format==TRIK_VIDTRANSCODE_CV_VIDEO_FORMAT_YUV422){   //T0D0 find condition   
//separate Cb Cr
      y2   = reinterpret_cast<uint8_t*>(s_y2);
      cb   = reinterpret_cast<uint8_t*>(s_cb);
      cr   = reinterpret_cast<uint8_t*>(s_cr);
...
//Sobel edge detection
      y_in_sobel  = reinterpret_cast<const unsigned char*>(s_y2);
      sobel_out = reinterpret_cast<unsigned char*>(s_y);
...
//Sobel edge detection
      y_in_sobel  = reinterpret_cast<const unsigned char*>(_inImage.m_ptr);
      sobel_out = reinterpret_cast<unsigned char*>(s_y);
      IMG_sobel_3x3_8(y_in_sobel, sobel_out, width, height);

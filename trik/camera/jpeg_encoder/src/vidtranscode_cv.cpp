...
#define IF_IN_OUT_FORMAT(_CVAlgorithm, _inFormat, _outFormat) \
  ...
  IF_IN_OUT_FORMAT(trik::cv::JPGEncoderWrapper, TRIK_VIDTRANSCODE_CV_VIDEO_FORMAT_YUV422, TRIK_VIDTRANSCODE_CV_VIDEO_FORMAT_RGB565X);
...
{
  static TRIK_VIDTRANSCODE_CV_Params s_default = {
  ...
  if(_params->base.formatInput==TRIK_VIDTRANSCODE_CV_VIDEO_FORMAT_YUV422)
    s_default.base.formatInput=TRIK_VIDTRANSCODE_CV_VIDEO_FORMAT_YUV422;
  else
    s_default.base.formatInput=TRIK_VIDTRANSCODE_CV_VIDEO_FORMAT_YUV422P;
